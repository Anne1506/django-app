...
from student_management_app.models import ExtraCurricular
...
def student_extracurricular(request):
    student = Students.objects.get(admin=request.user.id)
    extracurricular_data = ExtraCurricular.objects.filter(student=student)  # All statuses
    context = {
        "extracurricular_data": extracurricular_data,
    }
    return render(request, "student_template/student_extracurricular.html", context)

def student_extracurricular_add(request):
    if request.method == "POST":
        # Assuming you want to add a new extracurricular activity
        extracurricular_name = request.POST.get('extracurricular_name')
        student = Students.objects.get(admin=request.user.id)
        
        # Create a new extracurricular activity for the student
        new_extracurricular = ExtraCurricular(student=student, name=extracurricular_name)
        new_extracurricular.save()
        
        messages.success(request, "Extracurricular activity added successfully!")
        return redirect('student_extracurricular')  # Redirect to the list page (or appropriate page)
    
    return render(request, 'student_template/student_extracurricular_add.html')
